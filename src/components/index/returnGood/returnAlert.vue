<template>
    <div class="returnAlert" v-bind:style="{'top': (height / 2) + 'px', 'left': (width / 2) + 'px'}" v-loading="loading">
      <i>请输入退货商品信息</i>
      <el-form :inline="true" :model="ruleForm" :rules="rules" ref="ruleForm" class="demo-form-inline">
        ...
        <el-form-item class="button">
          <el-button class="button1" type="success" v-on:click="onSubmit('ruleForm')" v-bind:style="{'width': '180px'}">退货</el-button>
          <el-button class="button2" type="info" v-on:click="$emit('closeAndReturn')" v-bind:style="{'width': '180px'}">取消</el-button>
        </el-form-item>
      </el-form>
    </div>
</template>
    ...
    onSubmit (ruleForm) {
      this.$refs[ruleForm].validate((valid, field) => {
        if (valid) {
          ...
          this.$axios({
            method: 'post',
            url: `/v1/product/salesReturn/${this.ruleForm.number}`,
            auth: {
            ...
            this.loading = false
            this.$emit('closeAndReturnSucceed')
            if (res.status === 201) {
              if (res.data.error_code === 0) {
                this.$message.success('商品退货成功')
              }
            } else if (res.status === 404) {
              this.$message.error('商品编号不存在')
            } else {
              this.$message.error('退货数量超出销售数量')
            }
          })
        } else {
          setTimeout(() => {
